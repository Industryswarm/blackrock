...
var TestScheduler = (function (_super) {
    ...
    TestScheduler.prototype.materializeInnerObservable = function (observable, outerFrame) {
        ...
        observable.subscribe(function (value) {
            messages.push({ frame: _this.frame - outerFrame, notification: Notification_1.nextNotification(value) });
        }, function (error) {
            messages.push({ frame: _this.frame - outerFrame, notification: Notification_1.errorNotification(error) });
        }, function () {
            messages.push({ frame: _this.frame - outerFrame, notification: Notification_1.COMPLETE_NOTIFICATION });
        });
        return messages;
    };
    TestScheduler.prototype.expectObservable = function (observable, subscriptionMarbles) {
        ...
        this.schedule(function () {
            subscription = observable.subscribe(function (x) {
                ...
                actual.push({ frame: _this.frame, notification: Notification_1.nextNotification(value) });
            }, function (error) {
                actual.push({ frame: _this.frame, notification: Notification_1.errorNotification(error) });
            }, function () {
                actual.push({ frame: _this.frame, notification: Notification_1.COMPLETE_NOTIFICATION });
            });
        }, subscriptionFrame);
        ...
        var _loop_2 = function (i) {
            ...
            switch (c) {
                ...
                case ')':
                    ...
                    break;
                case '|':
                    notification = Notification_1.COMPLETE_NOTIFICATION;
                    advanceFrameBy(1);
                    break;
                case '^':
                    advanceFrameBy(1);
                    break;
                case '#':
                    notification = Notification_1.errorNotification(errorValue || 'error');
                    advanceFrameBy(1);
                    break;
                default:
                    if (runMode && c.match(/^[0-9]$/)) {
                    ...
                    notification = Notification_1.nextNotification(getValue(c));
                    advanceFrameBy(1);
                    break;
            }
            if (notification) {
    ...
    TestScheduler.prototype.createAnimator = function () {
        ...
        var delegate = {
            requestAnimationFrame: function (callback) {
                ...
                var handle = ++animationFramesHandle;
    ...
    TestScheduler.prototype.run = function (callback) {
        ...
        requestAnimationFrameProvider_1.requestAnimationFrameProvider.delegate = animator.delegate;
        ...
        try {
            ...
            this.runMode = false;
            requestAnimationFrameProvider_1.requestAnimationFrameProvider.delegate = undefined;
